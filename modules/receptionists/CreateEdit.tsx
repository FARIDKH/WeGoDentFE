...
const CreateEditForm = forwardRef(({ onSuccess }: IProps, ref) => {
    ...
    const { isLoading, mutate } = useMutation(
        async (values: any) => {
            // 1. Send a POST request to /api/account
            const response = await axios.post('/api/account', values);
            
            // Extract account ID from response
            const accountId = response?.data?.id;
    
            // 2. Pass that id to post clinic /clinicId/receptionists
            return axios.post(`/api/clinics/${data?.clinicId}/receptionists`,  [accountId] );
        },
        {
            onSuccess: () => {
                store.dispatch({
                    type: SNACKBAR_OPEN,
                    open: true,
                    message: data?.id ? 'Receptionist has been updated successfully' : 'Receptionist has been created successfully',
                    variant: 'alert',
        ...
        <Dialog sx={{ '& .MuiDialog-paper': { width: '30%', maxHeight: 600 } }} maxWidth="lg" open={isOpen}>
            <Formik
                ...
                onSubmit={(values) => {
                    const payload = {
                        ...
                        email: values.email,
                        roleIds: [7],  // set to receptionists role id
            ...
            >
                ...
                <form noValidate onSubmit={handleSubmit}>
                    <DialogTitle>
                        <span style={{ fontSize: 22, fontWeight: 'bold' }}>{data?.id ? 'Update' : 'Create'} receptionist</span>
                    </DialogTitle>
